...
import datetime, hashlib, json, re, sqlite3, subprocess, sys
...
source_requirements = {
    ...
    "REMOTE-DASHBOARD": (all(token in remote for token in ["remote_dashboard_access_logs", "HttpStatus.unauthorized", "stop"]), "Token-protected read-only service with access logging"),
...
flutter_path = subprocess.run(["bash", "-lc", "command -v flutter || true"], text=True, capture_output=True).stdout.strip()
dart_path = subprocess.run(["bash", "-lc", "command -v dart || true"], text=True, capture_output=True).stdout.strip()
toolchain = {"flutter": flutter_path, "dart": dart_path, "platform": sys.platform}
record("FLUTTER-RUNTIME-VALIDATION", "BLOCKED" if not flutter_path else "MISSING", json.dumps(toolchain, sort_keys=True))
record("WINDOWS-RUNTIME-VALIDATION", "BLOCKED" if sys.platform != "win32" else "MISSING", f"host_platform={sys.platform}; requires Windows 10/11 build host")
